...
public class RecordController {
  ...
  public MahasiswaModel[] prepend(MahasiswaModel[] record, MahasiswaModel data) {
    ...
    if (record_length > 0)
      System.arraycopy(record, 0, result, 1, record_length);
    result[0] = data;
  ...
  /**
   * Method untuk menambahkan data ke rekaman data utama sesuai dengan index yang
   * ditentukan
   *
   * @param record   Rekaman data utama
  ...
  public MahasiswaModel[] insert(MahasiswaModel[] record, int position, MahasiswaModel data) {
    ...
    if (position == 0) {
      result = this.prepend(record, data);
    } else if (position == record_length) {
      result = this.append(record, data);
    } else {
      for (int i = 0; i < result.length; i++)
        result[i] = i == position ? data : record[i < position ? i : i - 1];
    }
  ...
  public MahasiswaModel[] append(MahasiswaModel[] record, MahasiswaModel data) {
    ...
    MahasiswaModel[] result = new MahasiswaModel[record_length + 1];
    if (record_length > 0)
      System.arraycopy(record, 0, result, 0, record_length);
  ...
  /**
   * Method untuk menghapus data dari rekaman data utama sesuai dengan index yang
   * ditentukan
   * 
   * @param record   Rekaman data utama
  ...
  public DeleteResult delete(MahasiswaModel[] record, int position) {
    ...
    if (position == 0) {
      temp = this.shift(record).results;
    } else if (position == record_length) {
      temp = this.pop(record).results;
    } else {
      long start_time = System.nanoTime();
  ...
  public <T> MahasiswaModel[] binary_search(MahasiswaModel[] record, String prop, T keyword) {
    ...
    while (low <= high) {
      ...
      if (prop.equals("hobbies")) {
        ...
        val = this.value_from_model_prop(prop, record[mid]);
      }
